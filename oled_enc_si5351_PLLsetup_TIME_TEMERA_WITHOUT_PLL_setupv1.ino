  ...
  if (newPosition != oldPosition) {
    switch (menu) {
      ...
      case 5: //Настройка калибровки PLL
        if (newPosition > oldPosition && varinfo.calibration <= 50000) varinfo.calibration = varinfo.calibration + varinfo.stp / 10;
        if (newPosition < oldPosition && varinfo.calibration >= - 50000) varinfo.calibration = varinfo.calibration - varinfo.stp / 10;
        if (varinfo.calibration > 50000) varinfo.calibration = 50000;
        if (varinfo.calibration <  - 50000) varinfo.calibration =  - 50000;
        si5351.set_correction(varinfo.calibration * 10, SI5351_PLL_INPUT_XO);
        break;
